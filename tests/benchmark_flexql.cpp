...
static bool run_data_level_unit_tests(FlexQL *db) {
    ...
    record(run_exec(db, "CREATE TABLE TEST_USERS(ID DECIMAL, NAME VARCHAR(64), BALANCE DECIMAL, EXPIRES_AT DECIMAL);", "CREATE TABLE TEST_USERS"));

    auto insert_test_user = [&](long long id, const string &name, long long balance, long long expires_at) -> bool {
        stringstream ss;
        ss << "INSERT INTO TEST_USERS VALUES (" << id << ", '" << name << "', " << balance << ", " << expires_at << ");";
    ...
    record(q0);
    if (q0) record(assert_rows_equal("Basic SELECT * validation", rows, {"1|Alice|1200|1893456000", "2|Bob|450|1893456000", "3|Carol|2200|1893456000", "4|Dave|800|1893456000"}));
    ...
    record(run_exec(db, "CREATE TABLE TEST_ORDERS(ORDER_ID DECIMAL, USER_ID DECIMAL, AMOUNT DECIMAL, EXPIRES_AT DECIMAL);", "CREATE TABLE TEST_ORDERS"));
    ...
    bool q7 = query_rows(db, "SELECT TEST_USERS.NAME, TEST_ORDERS.AMOUNT FROM TEST_USERS INNER JOIN TEST_ORDERS ON TEST_USERS.ID = TEST_ORDERS.USER_ID WHERE TEST_ORDERS.AMOUNT > 900;", rows);
    record(q7);
    ...
    cout << "\nUnit Test Summary: " << passed_tests << "/" << total_tests << " passed, " << failed_tests << " failed.\n\n";

    return all_ok;
}

static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    if (!run_exec(db, "CREATE TABLE BIG_USERS(ID DECIMAL, NAME VARCHAR(64), EMAIL VARCHAR(64), BALANCE DECIMAL, EXPIRES_AT DECIMAL);", "CREATE TABLE BIG_USERS")) {
        return false;
    }
